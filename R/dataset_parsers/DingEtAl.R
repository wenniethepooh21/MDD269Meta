...
fullDingTable %>% write_csv(here("Processed_Data/DingEtAl/CompleteDingTable.csv"))
...
Ding_summary_results <- fullDingTable %>% DingMetaAnalysis(8, 4)
Ding_summary_results %>% write_csv(path = here("Processed_Data/DingEtAl/FullDingTable.csv"))

#Perform meta-analysis on all brain regions in males
#number of p-values we need to filter = 4 (4 regions * 1 sex) 
#number of brain regions = 4
Ding_Male <- fullDingTable %>% filter(sex == "male") 
...
#number of p-values we need to filter = 4 (4 regions * 1 sex) 
#number of brain regions = 4
Ding_Female <- fullDingTable %>% filter(sex == "female") 
#combine 4 (4 studies * 1 sexes) 
...
Ding_cortical <- fullDingTable %>% filter(brain_region != "AMY")
...
fullDing_flip <- fullDingTable %>% rowwise() %>% mutate(effectsize = if_else(sex == "male", effectsize*-1, effectsize))
full_Ding_flip_results <- fullDing_flip %>% DingMetaAnalysis(8,4)
...
full_Ding_flip_results_summary %>% write_csv(here("Processed_Data/DingEtAl/FullDingTable_flipped.csv"))
...
ding_magma <- read_csv(here("Raw_Data/HowardEtAl/FullMagmaGenes.csv")) %>% select(Ding_genes) %>% distinct() %>% na.omit()
...
Ding_summary_results%<>% right_join(ding_magma, by = c("gene_symbol" = "Ding_genes")) 
...
Ding_Female_results%<>% right_join(ding_magma, by = c("gene_symbol" = "Ding_genes")) 
...
Ding_Male_results%<>% right_join(ding_magma, by = c("gene_symbol" = "Ding_genes")) 
...
Ding_cortical_results%<>% right_join(ding_magma, by = c("gene_symbol" = "Ding_genes")) 

...
howard %<>% dplyr::select(gene_symbol, gene_symbol)
...
Howard_Table %<>% left_join(cns_max_cell_types %>% dplyr::select(mouse_gene,cns_cell_type_taxon), by = c('mouseGene' = 'mouse_gene'))
Howard_Table %<>% left_join(max_cell_types %>% dplyr::select(mouse_gene, cell_type_taxon), by = c('mouseGene' = 'mouse_gene'))

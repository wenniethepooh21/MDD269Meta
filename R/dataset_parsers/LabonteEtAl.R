...
Labonte_long <- bind_rows(Labonte %>% select(brain_region, gene_symbol, logFC = Male.logFC, pvalue = Male.pvalue) %>% mutate(sex = "male"),
                          Labonte %>% select(brain_region, gene_symbol, logFC = Female.logFC, pvalue = Female.pvalue) %>% mutate(sex = "female"))
Labonte_long %>% write_csv(here("Processed_Data/LabonteEtAl/CompleteLabonteTable.csv"))
...
#Extract Female data 
Labonte_Female <- Labonte %>% select(brain_region, gene_symbol, logFC = Female.logFC, pvalue = Female.pvalue) %>% mutate(sex = "female")
#Perform meta-analysis on all brain regions in female data 
#number of p-values we need to filter = 6 (6 regions * 1 sex) 
#number of brain regions = 6
Labonte_Female_results <- Labonte_Female %>% LabonteMetaAnalysis(6,6)
Labonte_Female_results %>% write_csv(here("Processed_Data/LabonteEtAl/FemaleLabonteTable.csv"))

#Extract Male data
Labonte_Male <- Labonte %>% select(brain_region, gene_symbol, logFC = Male.logFC, pvalue = Male.pvalue) %>% mutate(sex = "male")
#Perform meta-analysis on all brain regions in female data 
#number of p-values we need to filter = 6 (6 regions * 1 sex) 
#number of brain regions = 6
Labonte_Male_results <- Labonte_Male %>% LabonteMetaAnalysis(6,6)
...
Labonte_summary_full_flip %>% write_csv(here("Processed_Data/LabonteEtAl/FullLabonteTable_flipped.csv"))
...
labonte_magma <- read_csv(here("Raw_Data/HowardEtAl/FullMagmaGenes.csv")) %>% select(Labonte_genes) %>% distinct() %>% na.omit()
...
Labonte_summary_results %<>% right_join(labonte_magma, by = c('gene_symbol' = 'Labonte_genes'))
...
Labonte_Female_results %<>% right_join(labonte_magma, by = c('gene_symbol' = 'Labonte_genes'))
...
Labonte_Male_results%<>% right_join(labonte_magma, by = c('gene_symbol' = 'Labonte_genes'))
...
cortical_Labonte_summary_results%<>% right_join(labonte_magma, by = c('gene_symbol' = 'Labonte_genes'))

...
Howard %<>% dplyr::select(`Gene Name`, `P-value`)
Howard %<>% dplyr::rename(gene_symbol = `Gene Name`, Howard_pvalue = `P-value`)
...
full_gene_names %<>% left_join(gene_equivalent %>% dplyr::select(-gene_symbol), by = c('query' = 'Updated_Gene_Names'))

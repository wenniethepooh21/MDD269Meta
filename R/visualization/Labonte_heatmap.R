...
Labonte %<>% rowwise() %>% mutate(brain_region = if_else(brain_region == "Nac", "Nucleus_Accumbens", 
                                                if_else(brain_region == "Subic", "Subiculum", brain_region)),
...
title <- ggdraw() + draw_label("Labonte Gene Expression", fontface = "bold",size = 20)
# plot heatmap
print(plot_grid(title, labonte_plots,ncol = 1, rel_heights = c(0.1, 1)) )

ggsave(filename = here('Processed_Data/Meta_Analysis_Results/Heatmaps/top_genes_Labonte_expression_heatmap.png'), dpi=300, width=8, height=8)

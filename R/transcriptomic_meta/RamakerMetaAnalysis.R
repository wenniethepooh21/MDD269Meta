...
RamakerMetaAnalysis <- function(full_results, regions){
    #p-values are two tailed for negative and positive expression levels 
  	...
  	full_results %<>% mutate(higher_in_MDD_pvalue = two2one(P.Value, invert=1 == sign(-1*t))) #invert p-value if expression is lower (1-p_value) otherwise (p_value/2)
  	...
  	if(length(regions) > 1) {
  		summary_results <- full_results %>% group_by(gene_symbol) %>% summarize(min_p_across_regions = min(P.Value), 
  		  ...
  		  #there's only data for one brain region - use the p.values 
  			summary_results <- full_results %>% group_by(gene_symbol) %>% summarize(min_p_across_regions = min(P.Value), 
